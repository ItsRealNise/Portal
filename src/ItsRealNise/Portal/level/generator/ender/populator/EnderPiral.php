...
namespace ItsRealNise\Portal\level\generator\ender\populator;
...
/**
 * Class EnderPilar
 * @package ItsRealNise\Portal\level\generator\ender\populator
 */
                ...
                for($bx = -$bradius; $bx <= $bradius; $bx++){
                    for($by = -$bradius; $by <= $bradius; $by++){
                        for($bz = -$bradius; $bz <= $bradius; $bz++){
                            $edge = (($bx == $bradius || $bx == -$bradius) && ($bz == $bradius || $bz == -$bradius)) || ($by == $bradius || $by == -$bradius);
                            if($edge){
